...
export async function login(params: LoginParams): Promise<SkillResult> {
  try {
...
export async function logout(): Promise<SkillResult> {
  try {
    clearClient();
    return {
      success: true,
...
export async function getTeams(): Promise<SkillResult> {
  try {
    const client = getClient();
