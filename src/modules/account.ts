...
export async function getPublishPreset(params: { platformAccountId: string }): Promise<SkillResult> {
  try {
    const client = getClient();
    ...
    let message = "📋 发布预设信息:\n\n";
    if (result.videoCategory) message += `🎬 视频分类: ${result.videoCategory}\n`;
    if (result.videoTopics) message += `🎬 视频话题: ${result.videoTopics}\n`;
    if (result.articleCategory) message += `📝 文章分类: ${result.articleCategory}\n`;
    if (result.articleTopics) message += `📝 文章话题: ${result.articleTopics}\n`;
    if (result.dynamicCategory) message += `🖼️ 图文分类: ${result.dynamicCategory}\n`;
    if (result.dynamicTopics) message += `🖼️ 图文话题: ${result.dynamicTopics}\n`;
