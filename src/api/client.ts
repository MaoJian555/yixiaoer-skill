...
export class YixiaoerClient {
  ...
  private async loginInternal(username: string, password: string): Promise<void> {
    const response = await this.request<{ authorization: string }>('POST', '/users/auth/v2', {
      username,
      password
    });
  ...
  async getAccounts(params?: { page?: number; size?: number; loginStatus?: number }): Promise<{
    data: MediaAccount[];
    totalSize: number;
    page: number;
    size: number;
  ...
  async getPublishRecords(params?: { page?: number; size?: number }): Promise<any> {
  ...
  async getUploadUrl(fileName: string, fileSize: number, contentType: string): Promise<{
    uploadUrl: string;
    fileKey: string;
  }> {
    const result = await this.request<any>('GET', '/storages/cloud-publish/upload-url', {
      fileName,
      fileSize,
      contentType
    });
